...
def combine_tmax_rh(tmax, rh):
    ...
    tmax["Tasmax_F"] = tmax["Tasmax"].apply(
        lambda x: convert_kelvin_to_fahrenheit(x), meta=("Tasmax", "float64")
    )
...
def get_heat_index(ds_tasmax, ds_rh, shapefile):
    ...
    hi["heat_index"] = hi.apply(
        lambda x: calculate_heat_index(x["Tasmax_F"], x["RH_f_inst"]),
        meta=(None, "float64"),
        axis=1,
    )
